...
- **Консоль**: полноценный bash под пользователем `tcadmin` (root — через
  `sudo -i` с напоминанием о правилах), доступна из Settings → Console; при
  входе — памятка по `tc-edit`. Админ может запаролить весь раздел Settings
  (вместе с консолью) файлом `shell.pass` на флешке (см. «Использование»).
  На борту iproute2, ping, ethtool, tcpdump, lsusb, htop, mc, nano, vim, vi.
- **SSH**: sshd (dropbear) слушает 22-й порт с загрузки; вход — `tcadmin`
  (+`sudo`) или root. Пароли задаются **при сборке** в `site.env`, а не
  зашиты в репозиторий. Host-ключ персистится на флешке (нет «host key
  changed» на каждый коннект). Подробно — раздел «Пароль root и SSH».
...
- **Settings** — единый админ-раздел (при желании под паролем, см. ниже).
  ...
  - **Система**: *Console* (шелл обычного пользователя; root — `sudo -i`) и *Diagnostics* (интерфейсы/IP,
  ...
  - **Сеть и RDP**: *Network* (переключить DHCP ↔ статический IP/шлюз/DNS),
    *RDP defaults* (дефолт-логин/автоподключение), *Printer* (URI сетевого
...
- **Управление серверами** (Settings → Manage servers): список серверов;
  `a` или `+ Add server` — добавить (имя + адрес), `d` — быстрое удаление,
  `q`/Esc — назад. **Enter на сервере открывает его экран настроек** — пункты
  стрелками: *Name*, *Address*, *Security* (протокол **выбирается из списка**:
  auto / rdp / tls / nla, не вводится текстом), *Login* (логин этого сервера;
  домен — прямо в логине: `CORP\user`), *Delete server*. Настройки привязаны
  к серверу (протокол виден в списке как `[rdp]`), глобален только
  дефолт-логин.
  Изменения сразу пишутся на флешку; на read-only носителе (CD в VM) — нельзя.
...
- **Конфиг `tc.conf`** (необязательный): рядом с `servers.conf` на флешке.
  ...
  - `RDP_USER` — дефолтный логин окна входа RDP, если у сервера не задан
    свой (4-е поле в `servers.conf`). Домен, если нужен, пишется в логине:
    `CORP\user` или `user@corp.local`. Пароль пользователь вводит сам;
...
- **Консоль Linux**: Settings → Console — bash под обычным пользователем
  (по умолчанию `tcadmin`), при входе показывается памятка по `tc-edit`.
  Root — `sudo -i` (пароль пользователя; при переходе выводится короткое
  напоминание о правилах работы под root).
...
```sh
git clone https://github.com/elmarl0w/koschei-os && cd koschei-os
...
Без `site.env` собирается дефолт: `tcadmin`/`admin`, стартовый сервер из
...
Для входа по SSH в образ вшит пользователь (по умолчанию **`tcadmin`**/**`admin`**)
с правом **sudo** (`/etc/sudoers.d/thinclient`). Логин: `ssh tcadmin@<ip>` →
...
> ⚠️ Дефолт `tcadmin`/`admin` — **слабый пароль**, к тому же с полным sudo это
> фактически root по SSH. Для боевого парка задай свой в `site.env` (текстом
> или хэшем) и пересобери, либо `passwd tcadmin` в рантайме (эфемерно — система в
> RAM), либо сузь права в `sudoers.d`. Не оставляй `admin` в проде.
...
### Смена паролей — шпаргалка

| Пароль | Надёжно (переживает ребут) | Быстро (до ребута) |
|---|---|---|
| Пользователь (`tcadmin`) | `USER_PASSWORD=...` в `site.env` → `./build.sh` → перезалить образ | `passwd` под собой или `sudo passwd tcadmin` |
| root | `ROOT_PASSWORD=...` в `site.env` → пересборка; **или без пересборки** — файл `root.pass` на флешке с sha512-хэшем (`openssl passwd -6`) | `sudo passwd root` |
| Вход в Settings (меню) | `SETTINGS_PASSWORD=...` в `site.env` → пересборка; **или** правка `shell.pass` на флешке: `printf '%s' 'пароль' \| sha256sum \| cut -c1-64 > shell.pass` (через `sudo tc-edit shell.pass`) | то же — файл на флешке применяется сразу |

Система живёт в RAM: всё, что поменяно `passwd`, сбрасывается при
перезагрузке — надёжные пути только через `site.env`/файлы на флешке.
...
LICENSE                            GPL-2.0-or-later (текст GPLv2)
```
...
Код проекта — **GPL-2.0-or-later** ([текст GPLv2](LICENSE)): можно
использовать, менять и распространять, но производные при распространении
должны оставаться открытыми под той же лицензией (или, по выбору получателя,
любой более поздней версией GPL).

- Патчи в `patches/` — производные от патчуемых пакетов и наследуют их
  лицензии (psplash — GPLv2).
- **Собранные образы** (`usb.img`/ISO) — агрегат стороннего ПО под своими
  лицензиями: ядро Linux, busybox, syslinux, psplash — GPLv2; GRUB — GPLv3;
  FreeRDP, CUPS — Apache-2.0; glibc — LGPL; Xorg, ncurses, dropbear — MIT/X11
  и подобные. Полный манифест лицензий и архив исходников компонентов для
  конкретной сборки генерирует Buildroot: `make legal-info`.
- Версии до v1.3.0 включительно были опубликованы под MIT и остаются
  доступными на тех условиях.
