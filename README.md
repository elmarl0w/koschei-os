...
- **Управление серверами** (Settings → Manage servers): отдельный экран —
  ...
  подтверждение `y`), `q` — назад. При add/edit можно задать **протокол для
  этого сервера** (`rdp`/`tls`/`nla`, пусто = глобальный дефолт) — он виден в
  списке как `[rdp]` и переопределяет `RDP_SECURITY`. Пишется на флешку; на
  read-only носителе (CD в VM) изменить нельзя.
...
| `RDP_SECURITY` | протокол RDP по умолчанию: `auto`/`rdp`/`tls`/`nla` (`rdp` = без NLA/TLS) | `tc.conf` на флешке |
| `SERVERS` третье поле | протокол для КОНКРЕТНОГО сервера: `Имя=адрес;rdp` (переопределяет `RDP_SECURITY`) | `servers.conf` на флешке |
